...
int geo_latlng_deg(struct geonum_t num)
{
  uint32_t val;
  val = (num.secs >> 11) & 0xFFFFFF;
  return val / 3600;
}
int geo_latlng_min(struct geonum_t num)
{
  uint32_t val;
  val = (num.secs >> 11) & 0xFFFFFF;
  return (val / 60) % 60;
}
double geo_latlng_sec(struct geonum_t num)
{
  uint32_t val;
  val = (num.secs >> 11) & 0xFFFFFF;
  return (val % 60) + 0.125 * ((num.secs >> 8) & 0x7);
}

void showlatlong(struct geonum_t num, int islat)
{
  printf("%c%d %d %g ", 
	 num.secs & (1L << 31) ? (islat ? 'S' : 'W') : (islat ? 'N' : 'E'),
	 geo_latlng_deg(num),
	 geo_latlng_min(num),
	 geo_latlng_sec(num));
}
...
{
  showlatlong(pid.lat, 1);
  printf(" ");
  showlatlong(pid.lng, 0);
}
...
void showmid(struct mid_t mid)
{
  ...
  showdate(mid.date);
  printf(" ");
  showlatlong(mid.loc.lat, 1);
  printf(" ");
  showlatlong(mid.loc.lng, 0);
  printf("  floor:%d\n", mid.floor);
}
...
struct pdmdh_t
{
  ...
  struct pdmd_geo	upper_lat;
  struct pdmd_geo	lower_lat;
  struct pdmd_geo	left_lng;
  struct pdmd_geo	right_lng;
 ...
 *
 */
int xgeo_latlng_deg(struct pdmd_geo geo)
{
  uint32_t num;

  num = ((geo.secs[0] << 16) + (geo.secs[1] << 8) + (geo.secs[2] << 0)) & 0x7FFFFF;
  return num / (3600 * 8);
}

int xgeo_latlng_min(struct pdmd_geo geo)
{
  uint32_t num;
  num = ((geo.secs[0] << 16) + (geo.secs[1] << 8) + (geo.secs[2] << 0)) & 0x7FFFFF;
  return (num / (60 * 8)) % 60;
...
double xgeo_latlng_sec(struct pdmd_geo geo)
{
  uint32_t num;
  num = ((geo.secs[0] << 16) + (geo.secs[1] << 8) + (geo.secs[2] << 0)) & 0x7FFFFF;
  return (num % 60) / 8.0;
...
void printgeo(struct pdmd_geo geo, int islat)
{
  int flag;

  flag = (geo.secs[0] & (1L << 7));
  printf("%c%d %d %g\n",
	 (flag ? (islat ? 'S' : 'W') : (islat ? 'N' : 'E')),
	 xgeo_latlng_deg(geo),
	 xgeo_latlng_min(geo),
	 xgeo_latlng_sec(geo));
}
...
void showalldata()
{
  ...
  int nx[64] = { 0 };
  int ny[64] = { 0 };
  fd = open("../ALLDATA.KWI", O_LARGEFILE|O_RDONLY);
  if (fd < 0)
    return;
  read(fd, &dv, sizeof(dv));
  swapl(&dv.spec_mid.loc.lat.secs);
  swapl(&dv.spec_mid.loc.lng.secs);
  swapw(&dv.spec_mid.date);
  swapl(&dv.data_mid.loc.lat.secs);
  swapl(&dv.data_mid.loc.lng.secs);
  swapw(&dv.data_mid.date);
  swapl(&dv.system_mid.loc.lat.secs);
  swapl(&dv.system_mid.loc.lng.secs);
  swapw(&dv.system_mid.date);
  swapl(&dv.box_ll.lat.secs);
  swapl(&dv.box_ll.lng.secs);
  swapl(&dv.box_ur.lat.secs);
  swapl(&dv.box_ur.lng.secs);
  swapw(&dv.log_size);
  ...
  printgeo(pdmdh->upper_lat, 1);	
  printgeo(pdmdh->lower_lat, 1);	
  printgeo(pdmdh->left_lng, 0);	
  printgeo(pdmdh->right_lng, 0);	
...
int main(int argc, char *argv[])
{
  ...
  for (i=0; i<lmm.n; i++) {
    ...
    swapl(&sii[i].mid.loc.lat.secs);
    swapl(&sii[i].mid.loc.lng.secs);
    printf("sii%d: nm=%d\n", i+1, sii[i].nm);
    showmid(sii[i].mid);
